// ** Style Import
import styles from './index.module.scss';

// ** React Import
import { ReactNode, useEffect, useState } from 'react';

type Items = {
  id: string;
  name: string;
};

type DisplayGroup = 'column' | 'row';
type LabelDisplay = 'first' | 'last';

type CheckboxGroupProps = {
  name?: string;
  title: string;
  items: Items[];
  value?: string[];
  children?: ReactNode;
  displayGroup?: DisplayGroup;
  labelDisplay?: LabelDisplay;
  onChange: (value: string[]) => void;
};

function CheckboxGroup({
  children,
  name = '',
  items = [],
  title = '',
  value = [''],
  labelDisplay = 'first',
  displayGroup = 'column',
  onChange = () => {},
}: CheckboxGroupProps) {
  // ** State
  const [selectedValues, setSelectedValues] = useState<string[]>(value || []);

  // ** Effect
  useEffect(() => {
    setSelectedValues(value || []);
  }, [value]);

  // ** Function
  const handleCheckbox = (id: string) => {
    let updated: string[] = [];
    if (selectedValues.includes(id)) {
      updated = selectedValues.filter((item) => item !== id);
    } else {
      updated = [...selectedValues, id];
    }
    setSelectedValues(updated);
    onChange(updated);
  };

  // ** Icon
  const CheckedIcon = () => (
    <svg
      width='24px'
      height='24px'
      version='1.1'
      viewBox='0 0 24.00 24.00'
      xmlns='http://www.w3.org/2000/svg'
      xlinkHref='http://www.w3.org/1999/xlink'
      fill='#2569FF'
    >
      <g id='SVGRepo_bgCarrier' strokeWidth='0'></g>
      <g
        stroke='#CCCCCC'
        strokeWidth='0.144'
        strokeLinecap='round'
        strokeLinejoin='round'
        id='SVGRepo_tracerCarrier'
      ></g>
      <g id='SVGRepo_iconCarrier'>
        <title>ic_fluent_checkbox_checked_24_filled</title>
        <desc>Created with Sketch.</desc>
        <g
          fill='none'
          stroke='none'
          strokeWidth='1'
          fillRule='evenodd'
          id='🔍-Product-Icons'
        >
          <g
            fill='#2569FF'
            fillRule='nonzero'
            id='ic_fluent_checkbox_checked_24_filled'
          >
            <path
              d='M18,3 C19.6568542,3 21,4.34314575 21,6 L21,18 C21,19.6568542 19.6568542,21 18,21 L6,21 C4.34314575,21 
                    3,19.6568542 3,18 L3,6 C3,4.34314575 4.34314575,3 6,3 L18,3 Z M16.4696699,7.96966991 L10,14.4393398 L7.53033009,
                    11.9696699 C7.23743687,11.6767767 6.76256313,11.6767767 6.46966991,11.9696699 C6.1767767,12.2625631 6.1767767,
                    12.7374369 6.46966991,13.0303301 L9.46966991,16.0303301 C9.76256313,16.3232233 10.2374369,16.3232233 10.5303301,
                    16.0303301 L17.5303301,9.03033009 C17.8232233,8.73743687 17.8232233,8.26256313 17.5303301,7.96966991 C17.2374369,
                    7.6767767 16.7625631,7.6767767 16.4696699,7.96966991 Z'
              id='🎨-Color'
            ></path>
          </g>
        </g>
      </g>
    </svg>
  );

  const UncheckedIcon = () => (
    <svg
      width='24px'
      height='24px'
      version='1.1'
      viewBox='0 0 24 24'
      xmlns='http://www.w3.org/2000/svg'
      xlinkHref='http://www.w3.org/1999/xlink'
    >
      <title>ic_fluent_checkbox_unchecked_24_filled</title>
      <desc>Created with Sketch.</desc>
      <g
        fill='none'
        stroke='none'
        strokeWidth='1'
        fillRule='evenodd'
        id='🔍-Product-Icons'
      >
        <g
          fill='#212121'
          fillRule='nonzero'
          id='ic_fluent_checkbox_unchecked_24_filled'
        >
          <path
            d='M6,3 L18,3 C19.6568542,3 21,4.34314575 21,6 L21,18 C21,19.6568542 19.6568542,21 18,21 L6,21 C4.34314575,21 
                  3,19.6568542 3,18 L3,6 C3,4.34314575 4.34314575,3 6,3 Z M6,5 C5.44771525,5 5,5.44771525 5,6 L5,18 C5,18.5522847 
                  5.44771525,19 6,19 L18,19 C18.5522847,19 19,18.5522847 19,18 L19,6 C19,5.44771525 18.5522847,5 18,5 L6,5 Z'
            id='🎨Color'
          ></path>
        </g>
      </g>
    </svg>
  );

  return (
    <div className={styles.checkboxGroupContainer}>
      <div className={styles.checkboxGroupContainer__title}>{children}</div>
      <div
        className={`${styles.checkboxGroupContainer__style} ${
          displayGroup === 'column'
            ? styles['checkboxGroupContainer__style--column']
            : styles['checkboxGroupContainer__style--row']
        }`}
        aria-label={title}
      >
        {items.map((item) => {
          const checked = selectedValues.includes(item.id);
          return (
            <div key={item.id} className={styles.checkboxGroupContainer__list}>
              {labelDisplay === 'first' && (
                <label
                  htmlFor={`${name}-${item.id}`}
                  className={styles.checkboxGroupContainer__label}
                >
                  {item.name}
                </label>
              )}

              <div className={styles.checkboxGroupContainer__input}>
                <input
                  id={`${name}-${item.id}`}
                  name={name}
                  type='checkbox'
                  value={item.id}
                  checked={checked}
                  onChange={() => handleCheckbox(item.id)}
                />
                {checked ? <CheckedIcon /> : <UncheckedIcon />}
              </div>

              {labelDisplay === 'last' && (
                <label
                  htmlFor={`${name}-${item.id}`}
                  className={styles.checkboxGroupContainer__label}
                >
                  {item.name}
                </label>
              )}
            </div>
          );
        })}
      </div>
    </div>
  );
}
export default CheckboxGroup;
